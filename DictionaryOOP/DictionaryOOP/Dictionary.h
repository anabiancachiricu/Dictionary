...
Dictionary<K, V>::Dictionary()		//constructor fara parametrii
{
	root = NULL;
	noElements = 0;
}
...
template<class K, class V>
Node<K, V>* Dictionary<K, V>::copyNode(Node<K, V>* n) const		//functie care copiaza un nod
{
	if (n != NULL)
	{
...
template<class K, class V>			
Dictionary<K, V>::Dictionary(const Dictionary& d)		//constructor de copiere
{
	root = NULL;
...
template<class K, class V>
Dictionary <K, V>& Dictionary<K, V>:: operator=(const Dictionary& d)		//supraincarcarea operatorului egal pentru dictionare
{
	clearDictionary();
...
template<class K, class V>
void Dictionary<K, V>::clearNode(Node <K, V>*& n)		//functie care sterge tot de la un anumit nod in jos, inclusiv nodul
{
	if (n != NULL)
	{
		clearNode(n->left);
...
template<class K, class V>
void Dictionary<K, V>::clearDictionary()		//functie care sterge tot dictionarul, stergand fiecare nod pe rand
{
	clearNode(root);
}

template<class K, class V>
Node<K, V>* Dictionary<K, V>::RotateLeft(Node<K, V>* x)
{
	Node<K, V>* y = x->right;
	Node<K, V>* z = y->left;
...
template<class K, class V>
Node <K, V>* Dictionary<K, V>::RotateRight(Node<K, V>* y)
{
	Node<K, V>* x = y->left;
	Node<K, V>* z = x->right;
...
template<class K, class V>
Node<K, V>* Dictionary<K, V>::addNode(K& key, V& value, Node<K, V>*& n)
{
	if (n == NULL)
	{
		n = new Node<K, V>;
...
template<class K, class V>
int Dictionary<K, V>::getNoElements()
{
	return this->noElements;
}

template<class K, class V>
Node<K, V> Dictionary<K, V>::minValueNode(Node<K, V>* n)
{
	Node<K, V>* current = n;
...
template<class K, class V>
Node<K, V> Dictionary<K, V>::deleteNode(Node<K, V> root, K key)
{
...
template<class K, class V>
void Dictionary<K, V>::deleteNodeFromKey(K k)
{
	root = deleteNode(root, k);
	noElements--;
}
template<class K, class V>
V Dictionary<K, V>:: find(K k, Node<K, V> n)
{
	if (root->key == k)
...
template<class K, class V>
V Dictionary<K, V>::findKeyValue(K k)
{
	return find(k, root);
}
template<class K, class V>
V Dictionary<K, V>:: operator[](const K k)
{
	K key = k;
...
template<class K, class V>
void Dictionary<K, V>::inOrder(Node<K, V>* n, ostream &out)
{
	if (n != NULL)
	{
		inOrder(n->left);
